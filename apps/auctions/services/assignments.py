...
def auto_select_closed_winner(*, auction: Auction) -> Bid | None:
    """
    Automatically select the winner bid for a finished CLOSED auction.

    Winner = bid with the highest amount.
    Tie-breaker = earliest created_at (first to bid wins).

    Sets auction.winner_bid and auction.shortlisted_bids. The actual deal
    is created only after the owner confirms the auction result via
    confirm-result endpoint (ТЗ 8.2).
    """
...
def select_closed_auction_winner(*, auction: Auction, broker_id: int) -> Bid:
    """
    Manually select a single winner bid for a finished CLOSED auction.

    Sets auction.winner_bid and auction.shortlisted_bids. The deal is NOT
    created here — the owner must explicitly confirm the result via
    confirm-result endpoint (ТЗ 8.2).
    """
    if auction.mode != Auction.Mode.CLOSED:
