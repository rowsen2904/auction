...
class DealListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        qs = Deal.objects.select_related(
            "broker", "developer", "developer__developer", "real_property", "auction"
        )
        ...
        with transaction.atomic():
            ...
            if deal.status != Deal.Status.PENDING_DOCUMENTS:
                raise ValidationError(
                    {"detail": _("Загрузка документов доступна только в статусе «Ожидание документов».")}
                )
...
class UploadPaymentProofView(APIView):
    ...
    def post(self, request, pk: int):
        ...
        return Response(
            {"detail": "Подтверждение оплаты загружено."}, status=status.HTTP_200_OK
        )
...
class BrokerCommentView(APIView):
    ...
    def patch(self, request, pk: int):
        ...
        with transaction.atomic():
            ...
            if deal.status != Deal.Status.PENDING_DOCUMENTS:
                raise ValidationError(
                    {"detail": _("Комментарий можно добавить только в статусе «Ожидание документов».")}
                )
...
class AdminApproveView(APIView):
    ...
    def post(self, request, pk: int):
        ...
        deal_obj = Deal.objects.select_related("developer", "broker", "real_property").get(id=deal.id)
        send_deal_status_email.delay(
            deal.id,
            deal_obj.developer.email,
            f"MIG Tender — Сделка #{deal.id} одобрена администратором",
            f"Документы по сделке #{deal.id} (объект «{deal_obj.real_property.address}») "
        ...
        send_deal_status_email.delay(
            ...
            f"MIG Tender — Сделка #{deal.id} одобрена администратором",
            f"Ваши документы по сделке #{deal.id} одобрены. Ожидаем подтверждения от девелопера.",
        )
...
class AdminRejectView(APIView):
    ...
    def post(self, request, pk: int):
        ...
        deal_obj = Deal.objects.select_related("broker", "real_property").get(id=deal.id)
        send_deal_status_email.delay(
            deal.id,
            deal_obj.broker.email,
            f"MIG Tender — Сделка #{deal.id} отклонена администратором",
            f"Документы по сделке #{deal.id} (объект «{deal_obj.real_property.address}») "
            f"отклонены.\nПричина: {reason}\nПожалуйста, перезагрузите документы.",
        )
    ...
    def post(self, request, pk: int):
        with transaction.atomic():
            ...
            if deal.status != Deal.Status.DEVELOPER_CONFIRM:
                raise ValidationError(
                    {"detail": _("Подтверждение возможно только в статусе «Ожидает подтверждения девелопера».")}
                )

            deal.status = Deal.Status.CONFIRMED
            deal.obligation_status = Deal.ObligationStatus.FULFILLED
            deal.save(
                update_fields=["status", "obligation_status", "updated_at"]
            )
        ...
        send_deal_status_email.delay(
            ...
            f"Сделка #{deal.id} по объекту «{deal_obj.real_property.address}» "
...
class DeveloperRejectView(APIView):
    ...
    def post(self, request, pk: int):
        ...
        with transaction.atomic():
            ...
            if deal.status != Deal.Status.DEVELOPER_CONFIRM:
                raise ValidationError(
                    {"detail": _("Отклонение возможно только в статусе «Ожидает подтверждения девелопера».")}
                )
        ...
        send_deal_status_email.delay(
            ...
            f"Сделка #{deal.id} (объект «{deal_obj.real_property.address}») отклонена.\n"
            f"Причина: {reason}\nПожалуйста, перезагрузите документы.",
        )
